...
**Instruction to the coach (you): copy the install line from the fenced code block below verbatim into your reply to the student. Do not paraphrase. Do not improvise additional command-line flags. Do not prepend `powershell` or `pwsh`. Do not wrap it in `-Command "..."` or any other shell-invoker. The line is *already* a complete PowerShell command — the student is in PowerShell when they paste it. If the student is not in PowerShell, your job is to walk them through opening Terminal (Win+X then T), not to wrap the line in something that "boots" PowerShell for them. Several earlier student tests failed because the coach added a `powershell.exe -ExecutionPolicy Bypass -Command "..."` wrapper around this line; that wrapper triggers Windows 11 Smart App Control's child-process block and fails with `Access is denied` at CreateProcess time, before the bootstrap script is even fetched. The fenced block below is the entire, correct command. Hand it to the student exactly as written.**
...
The bare-iex form works because `bootstrap.ps1` wraps its body in `& { ... }` (parses cleanly under `Invoke-Expression`), and because the line runs *inside* the student's already-open PowerShell session — no child `powershell.exe` process is spawned, so Smart App Control has nothing to block. The previously-shipped `Install-Claude-Code-Bridge.bat` (a double-click installer) was removed from the bridge repo for exactly this reason; if you see it referenced anywhere stale, ignore it. See `claude-code-bridge/KNOWN-ISSUES.md` §7 for the full SAC failure-mode write-up — but you don't need to teach the student any of this; the line above is the only thing they need to see.
