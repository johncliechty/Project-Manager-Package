...
Read each box. For each, check the project state. Mark it ✓ or ✗. **All five must be ✓ to advance to M1** (six when the coach is running in a sandboxed runtime — see the bridge box below).

- [ ] **Cowork is running.** The student is in the chat with you. (Implicit pass — if you're reading this rubric, this box is green.)
- [ ] **The curriculum repo is cloned somewhere on disk.** Typical location is `~/Project-Manager-Package/` (Windows: `C:\Users\<name>\Project-Manager-Package\`). Verify the SKILL.md is readable at `<clone>/plugins/anchor-coach/skills/anchor-coach/SKILL.md`. If the clone is missing, run `git clone https://github.com/johncliechty/Project-Manager-Package <chosen-path>` via bash and re-check. *Coach-facing note for Cowork-sandboxed runs: a direct `git clone` into the student's mounted project folder may fail with "Operation not permitted" on some `.git/` internal files — the mount filesystem rejects them. Workaround: clone to `/tmp/Project-Manager-Package` inside the sandbox, then `cp -r --no-clobber` the working-tree files into the project folder. The student's `.git/` metadata won't survive but the curriculum content (the only thing we need) does. If the workaround leaves a staging folder you can't `rm` (same mount restriction), do not surface it to the student — system-setup housekeeping is coach-facing only per STYLE.md.*
...
- [ ] **Bridge IPC is reachable from this session (sandboxed runtimes only — skip on non-sandboxed agents).** Two conditions, both required: (a) the bridge folder (`C:\dev\claude-code-bridge\` on Windows, `~/dev/claude-code-bridge/` on macOS/Linux) is in the current session's mount set; (b) a ping round-trip — write `{"command": "Write-Output bridge-alive", "shell": "powershell"}` to `<bridge_root>/ipc/inbox/<uuid>.json` (atomic via `.tmp` + rename), poll `<bridge_root>/ipc/outbox/<uuid>.json` for the response — returns `exit_code: 0` within ~1 second. *Mount but no ping* means the daemon isn't running on the host — check the Scheduled Task `ClaudeCodeBridgeWatcher` via the use-claude-code paste-once flow. *No mount* means `request_cowork_directory` was never called for the bridge folder; call it now. *Neither* means run the bridge install paste from the "Required when sandboxed" section below, then complete both substeps. **M0 does not close until this box is green for sandboxed coaches**; otherwise every later host operation in the curriculum silently falls back to paste-once, which defeats the install.
...
**Mount the bridge folder in this session.** Before the verify ping can work, the coach must bring the bridge folder into the sandbox's mount set. Call `mcp__cowork__request_cowork_directory` with the bridge root path (`C:\dev\claude-code-bridge` on Windows, `~/dev/claude-code-bridge` on macOS/Linux). The user gets a one-click approval prompt; on approval the folder is now readable and writable from the coach's Read/Write/Edit/Grep/Glob tools and reachable in `bash` at the session's mount path (e.g. `/sessions/<id>/mnt/claude-code-bridge/...`). **Without this mount, the inbox folder is invisible to the sandbox and every host operation for the rest of the curriculum silently falls back to paste-once — undoing the whole reason for installing the bridge.** If the user denies the mount, surface that as a degraded-mode signal and fall back to the use-claude-code paste-once flow for host operations.

**Verify.** Once the install reports done AND the bridge folder is mounted in this session, ping the bridge with a `Write-Output bridge-alive` test request via the filesystem-IPC path documented in `plugins/use-claude-code/skills/use-claude-code/SKILL.md` (and in detail at `<bridge_root>/IPC-PROTOCOL.md` after the install completes). If the response comes back within ~1 second, the bridge is operational and the box is green. If the mount succeeded but the ping times out, the daemon may not be running — check `Get-ScheduledTaskInfo -TaskName ClaudeCodeBridgeWatcher` (Windows) via the use-claude-code paste-once flow, since the bridge itself is what's broken at that moment. If the mount itself was denied or the install fails repeatedly, surface the error and fall back to the use-claude-code paste-once flow as a degraded mode for the whole curriculum.
