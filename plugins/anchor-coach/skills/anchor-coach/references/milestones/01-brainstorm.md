...
- [ ] First git commit has happened with the message `M1: brief written`. The coach ran the commit via the Setup Protocol's commit step (`git init` if first time, then `git add . && git commit -m "M1: brief written"`) and reported the commit hash. If `auto_commit: false` in the student's `config.yaml`, the coach prompted once for confirmation before running. The student does not type git commands at any point.
